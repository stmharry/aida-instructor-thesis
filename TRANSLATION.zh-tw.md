...
* Input file: `main.en.typ`
* Output file: `main.zh-tw.typ`
...
3. **Math handling**
   ...
   * You may translate *math-adjacent prose* (sentences introducing equations).
...
Output a file named `main.zh-tw.typ` that compiles under Typst in the same way as the original, with only the intended text translated and all technical/structural elements preserved. There may be auxiliary files associated with the main manuscript. Translate them accordingly as well and place them under the `zh-tw/` directory.
